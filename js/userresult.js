...
function reportDate()
{
	$.post('php/getdate.php',{}, function(candid){
		candid = candid.split(',');
		Fromdate = candid[0];
		Todate = candid[1];
		if(Fromdate == Todate)
		{
			var out = "<h2 style='font-family: Tahoma;'><b>Survey Result as of "+Fromdate+"</b></h2>";
			document.getElementById("title").innerHTML = out;
		}
		else
		{
			var out = "<h2 style='font-family: Tahoma;'><b>Survey Result as of "+Fromdate+" to "+Todate+"</b></h2>";
			document.getElementById("title").innerHTML = out;
		}
		generate();
	});
